...
get_version_from_cargo() {
    ...
    grep -A5 '\[workspace.package\]' "$toml" | grep '^version' | head -1 | sed 's/version = "\(.*\)"/\1/' || \
        grep -m1 '^version' "$toml" | sed 's/version = "\(.*\)"/\1/'
}

# --- Workspace utilities ---------------------------------------------------

get_workspace_binaries() {
    local toml="${1:-$PROJECT_ROOT/Cargo.toml}"
    [[ ! -f "$toml" ]] && { echo "$PROJECT_NAME"; return 0; }

    local members
    members=$(grep -A20 '^\[workspace\]' "$toml" 2>/dev/null | \
              grep '^members' | head -1 | \
              sed 's/.*=\[//;s/\].*//;s/"//g;s/,/ /g')

    [[ -z "$members" ]] && { echo "$PROJECT_NAME"; return 0; }
    ...
    for member in $members; do
        local dir="$root/$member"
        if [[ -f "$dir/src/main.rs" ]] && [[ -f "$dir/Cargo.toml" ]]; then
            local name
            name=$(grep -m1 '^name' "$dir/Cargo.toml" 2>/dev/null | sed 's/name = "\(.*\)"/\1/')
            if [[ -n "$name" ]]; then
                binaries+=("$name")
                has_binary=true
            fi
...
generate_nfpm_yaml() {
    ...
    local bin_name="${3:-$PROJECT_NAME}"
    local binary_path="${4:-./target/release/${bin_name}}"
